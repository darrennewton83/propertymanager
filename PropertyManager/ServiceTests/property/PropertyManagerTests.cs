...
namespace ServiceTests.property
{
    public class PropertyManagerTests
    {
        ...
        public class DeleteAsync : Fixture
        {
            ...
            {
                var sut = new PropertyManager(this.PropertyDataStore);
                this.PropertyDataStore.DeleteAsync(2).Returns(true);
            ...
            {
                var sut = new PropertyManager(this.PropertyDataStore);
                this.PropertyDataStore.DeleteAsync(2).Returns(false);
                var result = await sut.DeleteAsync(2);
        ...
        public class GetAsync : Fixture
        {
            ...
            {
                var sut = new PropertyManager(Substitute.For<IPropertyDataStore>());
                this.PropertyDataStore.GetAsync(2).Returns(new Property(2, PropertyType.Apartment, new Address("line 1", "line 2", "city", "region", "postcode"), 100000, new DateOnly(2023, 02, 06), true, 2, "notes"));
            ...
            {
                var sut = new PropertyManager(Substitute.For<IPropertyDataStore>());
                this.PropertyDataStore.GetAsync(2).ReturnsNull();
                var result = await sut.GetAsync(2);
                Assert.Null(result);
            }
        }
        ...
        public class Fixture
        {
            ...
            public Fixture()
            {
                this.PropertyDataStore = Substitute.For<IPropertyDataStore>();
            }
        }
    }
}
