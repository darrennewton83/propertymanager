namespace PropertyManager.Shared.DatabaseIntegrationTests.property
{
    ...
    public class MicrosoftSqlServerPropertyDataStoreTests
    {
        [Collection("DatabaseCollection")]
        public class SaveAsync : Fixture
        {
            ...
            {
                var sut = new MicrosoftSqlServerPropertyDataStore(new MicrosoftSqlServer(this.DatabaseLogger, DatabaseFixture.ConnectionString), Logger);
        ...
        public class DeleteAsync : Fixture
        {
            ...
            public async void GivenRecordExists_DeleteAsync_ReturnsTrue()
            {
                ...
                var connection = new MicrosoftSqlServer(this.DatabaseLogger, DatabaseFixture.ConnectionString);
                var count = await connection.ExecuteScalarAsync("select COUNT(id) from property.overview where id = 2");
