...
namespace Service.Controllers
{
    ...
    public class PropertyTypeController : ControllerBase
    {
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<PropertyTypeApiDto>> Get(int id)
        {
            if (id <= 0)
            {
                return BadRequest();
            ...
            return new OkObjectResult(_mapper.Map<PropertyTypeApiDto>(propertyType));
        }
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Save([FromBody]  PropertyTypeApiDto propertyType)
        {
            if (propertyType == null)
            {
                return BadRequest();
            ...
            if (savedPropertyType.Type == EntityResults.ResultType.Error)
            {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Update([FromBody] PropertyTypeApiDto propertyType)
        {
            if (propertyType == null)
            {
                return BadRequest();
